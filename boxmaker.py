...
class BoxMaker(inkex.Effect):
  def __init__(self):
      ...
      self.OptionParser.add_option('--path', action='store',type='inkbool',
        dest='path',default=False,help='All the lines of a shape in just one path')
  ...
  def effect(self):
    ...
    for idx, piece in enumerate(pieces): # generate and draw each piece of the box
      ...
      #Si queremos que todo aparezca en un Unico path
      if path :
        sa = side((x,y),(d,a),(-b,a),atabs * (-thickness if a else thickness),dx,(1,0),a,0,(keydivfloor|wall) * (keydivwalls|floor) * divx*yholes*atabs,yspacing,divOffset)
        sb = side((x+dx,y),(-b,a),(-b,-c),btabs * (thickness if b else -thickness),dy,(0,1),b,0,(keydivfloor|wall) * (keydivwalls|floor) * divy*xholes*btabs,xspacing,divOffset)
        if atabs:
            sc = side((x+dx,y+dy),(-b,-c),(d,-c),ctabs * (thickness if c else -thickness),dx,(-1,0),c,0,0,0,divOffset)
        else:
            sc = side((x+dx,y+dy),(-b,-c),(d,-c),ctabs * (thickness if c else -thickness),dx,(-1,0),c,0,(keydivfloor|wall) * (keydivwalls|floor) * divx*yholes*ctabs,yspacing,divOffset)
        if btabs:
            sd = side((x,y+dy),(d,-c),(d,a),dtabs * (-thickness if d else thickness),dy,(0,-1),d,0,0,0,divOffset)
        else:
            sd = side((x,y+dy),(d,-c),(d,a),dtabs * (-thickness if d else thickness),dy,(0,-1),d,0,(keydivfloor|wall) * (keydivwalls|floor) * divy*xholes*dtabs,xspacing,divOffset)
      ...
      if idx==0:
        ...
        for n in range(0,divx): # generate X dividers
            ...
            #Si queremos que todo aparezca en un Unico path
            if path :
                sa=side((x,y),(d,a),(-b,a),keydivfloor*atabs*(-thickness if a else thickness),dx,(1,0),a,1,0,0,divOffset)
                sb=side((x+dx,y),(-b,a),(-b,-c),keydivwalls*btabs*(thickness if keydivwalls*b else -thickness),dy,(0,1),b,1,divy*xholes,xspacing,divOffset)
                sc=side((x+dx,y+dy),(-b,-c),(d,-c),keydivfloor*ctabs*(thickness if c else -thickness),dx,(-1,0),c,1,0,0,divOffset)
                sd=side((x,y+dy),(d,-c),(d,a),keydivwalls*dtabs*(-thickness if d else thickness),dy,(0,-1),d,1,0,0,divOffset)
                total = sa + sb[sb.find('L'):] + sc[sc.find('L'):] + sd[sd.find('L'):sd.rfind('L')] + ' Z'
                drawS(total)
            else :
